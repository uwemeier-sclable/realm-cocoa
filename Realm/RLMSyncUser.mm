     ...
     completion:^(NSError *error, NSDictionary *json) {
        ...
        for (NSDictionary *permission in json[@"permissions"]) {
            if ([permission[@"path"] hasPrefix:@"/__"]) {
                continue; // FIXME: tests need this?
            }
            // FIXME: key/value?
...
- (void)applyPermission:(RLMSyncPermission *)permission callback:(RLMPermissionStatusBlock)callback {
    if (NSError *error = check_user(_user, @"applied", RLMSyncPermissionErrorChangeFailed)) {
        callback(error);
        return;
    }
    id condition;
...
- (void)createOfferForRealmAtURL:(NSURL *)url
    ...
    if (NSError *error = check_user(_user, @"offered", RLMSyncPermissionErrorAcceptOfferFailed)) {
        callback(nil, error);
        return;
    }
    ...
    [RLMSyncOfferPermissionsEndpoint
     sendRequestToServer:self.authenticationServer
     JSON:@{kRLMSyncTokenKey: self.refreshToken,
            @"expiresAt": expirationDate ? [dateFormatter stringFromDate:expirationDate] : NSNull.null,
            @"realmPath": url.path,
...
- (void)acceptOfferForToken:(NSString *)token
    ...
    if (NSError *error = check_user(_user, @"accepted", RLMSyncPermissionErrorChangeFailed)) {
        callback(nil, error);
        return;
    }
...
- (void)invalidateOfferForToken:(NSString *)token
                       callback:(RLMPermissionOfferResponseStatusBlock)callback {
    if (NSError *error = check_user(_user, @"invalidated", RLMSyncPermissionErrorChangeFailed)) {
        callback(nil, error);
        return;
    }
